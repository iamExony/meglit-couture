    ...
    handler: async (ctx, { customerId, email }) => {
        ...
        if (customerId) {
            try {
                ...
                if (cust?.email)
                    emails.add(String(cust.email).toLowerCase());
            }
            catch {
        ...
        const filtered = all.filter((o) => {
            ...
            if (customerId && c.customerId === customerId)
                return true;
            if (emails.size > 0 && typeof c.email === "string" && emails.has(c.email.toLowerCase()))
                return true;
            return false;
        });
...
async function resolveByKey(ctx, key) {
    let o = await ctx.db
        .query("orders")
...
export const updateStatus = mutation({
    ...
    handler: async (ctx, { key, status }) => {
        const o = await resolveByKey(ctx, key);
        if (!o)
            return null;
