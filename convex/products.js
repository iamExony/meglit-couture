...
async function resolveImage(ctx, img) {
    if (!img)
        return img;
...
async function withResolvedImages(ctx, product) {
    if (!product)
        return product;
    const images = await Promise.all((product.images || []).map((i) => resolveImage(ctx, i)));
    ...
    handler: async (ctx, { items }) => {
        for (const item of items) {
            ...
            if (typeof item.productId === "number") {
                ...
                try {
                    prod = await ctx.db.get(item.productId);
                }
                catch { /* ignore */ }
            }
            if (!prod)
