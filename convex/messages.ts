...
export const threadsForUser = query({
  ...
  handler: async (ctx, { userId }) => {
    const all = await ctx.db.query("messages").collect();
    const partners = new Map<string, { partnerId: any; lastMessage: any; unread: number }>();
    for (const m of all) {
      const partnerId =
        m.fromId === userId ? m.toId : m.toId === userId ? m.fromId : null;
      if (!partnerId) continue;
      const key = partnerId.toString();
      const existing =
        partners.get(key) || { partnerId, lastMessage: null, unread: 0 };
      if (
      ...
      if (m.toId === userId && !m.readBy.includes(userId)) existing.unread += 1;
    ...
    for (const t of partners.values()) {
      ...
      if (partner) {
        const { passwordHash, salt, ...pub } = partner as any;
